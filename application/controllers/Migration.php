...
class Migration extends CI_Controller {
    ...
    public function insert_entries_existing_invoices_payments(){
        ...
        foreach($get_all_db as $db){
            ...
            $inv_data = $this->Mymodel->GetDataAllRecords('invoice',"invoice_date >= '2020-04-01'");
            foreach($inv_data as $inv){
                $bal = $this->Mymodel->GetDataCount('ledger', array('sum(dr_amount) as dr_amount', 'sum(cr_amount) as cr_amount'), "customer_id=".$inv->Customer_id);
                $op_bal = $this->Mymodel->GetDataCount('customers', array('Balance_as_on', 'Type', 'Opening_balance'), "id=".$inv->Customer_id);
                if($op_bal->Type == 'Dr'){
                     $balance_amt = ($op_bal->Opening_balance + $bal->dr_amount) - ($bal->cr_amount + $inv->Netammount);
                } else {
            ...
            //print_r($cus_data);exit;
            $payment_data = $this->Mymodel->GetDataAllRecords('invoice_payments',"payment_date >= '2020-04-01'");
            foreach($payment_data as $payment){
                $bal = $this->Mymodel->GetDataCount('ledger', array('sum(dr_amount) as dr_amount', 'sum(cr_amount) as cr_amount'), "customer_id=".$payment->customer_id);
                ...
                if($op_bal->Type == 'Dr'){
                     ...
                     $balance_amt = ($op_bal->Opening_balance + $bal->cr_amount) - ($bal->dr_amount + $payment->payed_amount);
                }

<?php if(!defined('BASEPATH')) exit('No direct script access allowed');
  ...
  */
  class Datatables
  {
    ...
    * @return mixed
    */
    public function edit_column($column, $content, $match_replacement, $date_format = "")
    {
      $this->edit_columns[$column][] = array('content' => $content, 'replacement' => $this->explode(',', $match_replacement), 'dt_dormat' => $date_format);
      return $this;
    }
    /**
    * Unset column
    ...
    private function produce_output($output, $charset)
    {
      ...
      foreach($rResult->result_array() as $row_key => $row_val)
      {
        ...
        foreach($this->edit_columns as $modkey => $modval){
          foreach($modval as $key => $val){
            if($val['dt_dormat'] != ''){
              $aaData[$row_key][($this->check_cType())? $modkey : array_search($modkey, $this->columns)] = date($val['dt_dormat'], strtotime($this->exec_replace($val, $aaData[$row_key])));
            } else {
...
/* Generated by Harviacode Codeigniter CRUD Generator 2020-08-31 18:12:50 */
/* http://harviacode.com */
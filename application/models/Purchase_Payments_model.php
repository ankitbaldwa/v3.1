...
class Purchase_Payments_model extends CI_Model
{
    ...
    {
        $feilds = array('i.id', 'c.FirstName', 'c.LastName', 'u.username', 'i.invoice_no', 'i.invoice_date', 'i.Netammount', 'i.Balance_Amount', 'i.Status', 'p.receipt_no', 'p.billed_amount', 'p.payed_amount', 'p.balance_amount as payment_bal_amt', 'p.payment_type', 'p.payment_description', 'p.status as payment_status', 'p.timestrap','p.id as payment_id','p.payment_date');
        $this->db->select($feilds);
        $this->db->from('purchase_invoice i');
        $this->db->join('suppliers c', 'i.Suppliers_id = c.id');
        $this->db->join('users u', 'i.user_id= u.id');
        $this->db->join('purchase_invoice_payments p', 'p.purchase_invoice_id = i.id');
    ...
    {
        $feilds = array('i.id', 'c.id as customer_id', 'c.FirstName', 'c.LastName','c.Address','c.City','c.State','c.Country', 'c.Zip','c.GST_No','c.Email','c.Mobile', 'u.username', 'i.State_code','i.invoice_no', 'i.invoice_date','i.Lorry_no','i.waybill','i.Place','i.Gross_amount','i.Additional_amount','i.CGST_percentage','i.SGST_percentage','i.IGST_percentage','i.CGST','i.SGST','i.IGST', 'i.Roundoff','i.Netammount', 'i.Balance_Amount','i.Timestamp', 'i.Status','i.user_id');
        $this->db->select($feilds);
        $this->db->from($table);
        $this->db->join('suppliers c', 'i.Suppliers_id = c.id');
        $this->db->join('users u', 'i.user_id= u.id');
